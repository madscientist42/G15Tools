 ...
 */
void
g15r_pixelReverseFill (render_context_t *rctx, int x1, int y1, int x2, int y2,
		       int fill, int color)
{
  int x = 0;
  int y = 0;
  ...
  for (x = x1; x <= x2; ++x)
	...
	{
	  if (!fill)
	    color = !g15r_getPixel (rctx, x, y);
	  g15r_setPixel (rctx, x, y, color);
	}
    }
}
 ...
 */
void
g15r_pixelOverlay (render_context_t *rctx, int x1, int y1, int width, int height,
		   short colormap[])
{
  int i = 0;
 ...
 */
void
g15r_drawLine (render_context_t *rctx, int px1, int py1, int px2, int py2,
	       const int color)
{
  /* 
 ...
 */
void
g15r_pixelBox (render_context_t *rctx, int x1, int y1, int x2, int y2, int color,
	       int thick, int fill)
{
  int i = 0;
  for (i = 0; i < thick; ++i)
    {
      g15r_drawLine (rctx, x1, y1, x2, y1, color);	/* Top    */
      g15r_drawLine (rctx, x1, y1, x1, y2, color);	/* Left   */
      g15r_drawLine (rctx, x2, y1, x2, y2, color);	/* Right  */
      g15r_drawLine (rctx, x1, y2, x2, y2, color);	/* Bottom */
      x1++;
      y1++;
      x2--;
      y2--;
 ...
 */
void
g15r_drawCircle (render_context_t *rctx, int x, int y, int r, int fill, int color)
{
  int xx, yy, dd;
  ...
  while (yy >= 0)
    {
      if (!fill)
	{
	  g15r_setPixel (rctx, x + xx, y - yy, color);
	  g15r_setPixel (rctx, x + xx, y + yy, color);
	  g15r_setPixel (rctx, x - xx, y - yy, color);
	  g15r_setPixel (rctx, x - xx, y + yy, color);
	}
	...
	{
	  g15r_drawLine (rctx, x - xx, y - yy, x + xx, y - yy, color);
	  g15r_drawLine (rctx, x - xx, y + yy, x + xx, y + yy, color);
	}
      if (dd + yy > 0)
	{
 ...
 */
void
g15r_drawRoundBox (render_context_t *rctx, int x1, int y1, int x2, int y2,
		   int fill, int color)
{
  int y, shave = 3;
  ...
  if ((x1 != x2) && (y1 != y2))
    {
      if (fill)
	{
	  g15r_drawLine (rctx, x1 + shave, y1, x2 - shave, y1, color);
	  for (y = y1 + 1; y < y1 + shave; y++)
	    g15r_drawLine (rctx, x1 + 1, y, x2 - 1, y, color);
	  for (y = y1 + shave; y <= y2 - shave; y++)
	    g15r_drawLine (rctx, x1, y, x2, y, color);
	  for (y = y2 - shave + 1; y < y2; y++)
	    g15r_drawLine (rctx, x1 + 1, y, x2 - 1, y, color);
	  g15r_drawLine (rctx, x1 + shave, y2, x2 - shave, y2, color);
	  if (shave == 4)
	    {
	      g15r_setPixel (rctx, x1 + 1, y1 + 1,
			     color ==
			     G15_COLOR_WHITE ? G15_COLOR_BLACK :
			     G15_COLOR_WHITE);
	      g15r_setPixel (rctx, x1 + 1, y2 - 1,
			     color ==
			     G15_COLOR_WHITE ? G15_COLOR_BLACK :
			     G15_COLOR_WHITE);
	      g15r_setPixel (rctx, x2 - 1, y1 + 1,
			     color ==
			     G15_COLOR_WHITE ? G15_COLOR_BLACK :
			     G15_COLOR_WHITE);
	      g15r_setPixel (rctx, x2 - 1, y2 - 1,
			     color ==
			     G15_COLOR_WHITE ? G15_COLOR_BLACK :
			     G15_COLOR_WHITE);
	    }
	...
	{
	  g15r_drawLine (rctx, x1 + shave, y1, x2 - shave, y1, color);
	  g15r_drawLine (rctx, x1, y1 + shave, x1, y2 - shave, color);
	  g15r_drawLine (rctx, x2, y1 + shave, x2, y2 - shave, color);
	  g15r_drawLine (rctx, x1 + shave, y2, x2 - shave, y2, color);
	  if (shave > 1)
	    {
	      g15r_drawLine (rctx, x1 + 1, y1 + 1, x1 + shave - 1, y1 + 1,
			     color);
	      g15r_drawLine (rctx, x2 - shave + 1, y1 + 1, x2 - 1, y1 + 1,
			     color);
	      g15r_drawLine (rctx, x1 + 1, y2 - 1, x1 + shave - 1, y2 - 1,
			     color);
	      g15r_drawLine (rctx, x2 - shave + 1, y2 - 1, x2 - 1, y2 - 1,
			     color);
	      g15r_drawLine (rctx, x1 + 1, y1 + 1, x1 + 1, y1 + shave - 1,
			     color);
	      g15r_drawLine (rctx, x1 + 1, y2 - 1, x1 + 1, y2 - shave + 1,
			     color);
	      g15r_drawLine (rctx, x2 - 1, y1 + 1, x2 - 1, y1 + shave - 1,
			     color);
	      g15r_drawLine (rctx, x2 - 1, y2 - 1, x2 - 1, y2 - shave + 1,
			     color);
	    }
	}
    }
 ...
 */
void
g15r_drawBar (render_context_t *rctx, int x1, int y1, int x2, int y2, int color,
	      int num, int max, int type)
{
  ...
  if (type == 1)
    {
      g15r_pixelBox (rctx, x1, y1 - type, x2, y2 + type, color ^ 1, 1, 1);
      g15r_pixelBox (rctx, x1, y1 - type, x2, y2 + type, color, 1, 0);
    }
  else if (type == 2)
    {
      g15r_pixelBox (rctx, x1 - 2, y1 - type, x2 + 2, y2 + type, color ^ 1,
		     1, 1);
      g15r_pixelBox (rctx, x1 - 2, y1 - type, x2 + 2, y2 + type, color, 1,
		     0);
    }
    ...
    {
      g15r_drawLine (rctx, x1, y1 - type, x1, y2 + type, color);
      g15r_drawLine (rctx, x2, y1 - type, x2, y2 + type, color);
      g15r_drawLine (rctx, x1, y1 + ((y2 - y1) / 2), x2,
		     y1 + ((y2 - y1) / 2), color);
    }
  g15r_pixelBox (rctx, x1, y1, (int) ceil (x1 + length), y2, color, 1, 1);
}
 ...
 */
int 
g15r_loadWbmpSplash(render_context_t *rctx, char *filename)
{
    ...
    memcpy (rctx->canvas->buffer, buf, G15_BUFFER_LEN);
    return 0;
}
 ...
 */
void 
g15r_drawIcon(render_context_t *rctx, char *buf, int my_x, int my_y, int width, int height)
{
    int y,x,val;
    ...
    for (y=0; y < height - 1; y++)
      for (x=0; x < width - 1; x++)
		...
		g15r_setPixel (rctx, x + my_x, y + my_y, val);
        }
}
 ...
 */
void 
g15r_drawSprite(render_context_t *rctx, char *buf, int my_x, int my_y, int width, int height, int start_x, int start_y, int total_width)
{
    int y,x,val;
    unsigned int pixel_offset = 0;
    unsigned int byte_offset, bit_offset;
 ...
 */ 
void 
g15r_drawBigNum (render_context_t *rctx, unsigned int x1, unsigned int y1, unsigned int x2, unsigned int y2, int color, int num)
{
    x1 += 2;
    x2 -= 2;
    ...
    switch(num){
        case 0:
            g15r_pixelBox (rctx, x1, y1, x2, y2 , color, 1, 1);
            g15r_pixelBox (rctx, x1 +5, y1 +5, x2 -5, y2 - 6, 1 - color, 1, 1);
            break;
        case 1: 
            g15r_pixelBox (rctx, x2-5, y1, x2, y2 , color, 1, 1);
            g15r_pixelBox (rctx, x1, y1, x2 -5, y2, 1 - color, 1, 1);
            break;
        case 2:
            g15r_pixelBox (rctx, x1, y1, x2, y2 , color, 1, 1);
            g15r_pixelBox (rctx, x1, y1+5, x2 -5, y1+((y2/2)-3), 1 - color, 1, 1);
            g15r_pixelBox (rctx, x1+5, y1+((y2/2)+3), x2 , y2-6, 1 - color, 1, 1);
            break;
        case 3:
            g15r_pixelBox (rctx, x1, y1, x2, y2 , color, 1, 1);
            g15r_pixelBox (rctx, x1, y1+5, x2 -5, y1+((y2/2)-3), 1 - color, 1, 1);
            g15r_pixelBox (rctx, x1, y1+((y2/2)+3), x2-5 , y2-6, 1 - color, 1, 1);
            break;
        case 4:
            g15r_pixelBox (rctx, x1, y1, x2, y2 , color, 1, 1);
            g15r_pixelBox (rctx, x1, y1+((y2/2)+3), x2 -5, y2, 1 - color, 1, 1);
            g15r_pixelBox (rctx, x1+5, y1, x2-5 , y1+((y2/2)-3), 1 - color, 1, 1);
            break;
        case 5:
            g15r_pixelBox (rctx, x1, y1, x2, y2 , color, 1, 1);
            g15r_pixelBox (rctx, x1+5, y1+5, x2 , y1+((y2/2)-3), 1 - color, 1, 1);
            g15r_pixelBox (rctx, x1, y1+((y2/2)+3), x2-5 , y2-6, 1 - color, 1, 1);
            break;
        case 6:
            g15r_pixelBox (rctx, x1, y1, x2, y2 , color, 1, 1);
            g15r_pixelBox (rctx, x1+5, y1+5, x2 , y1+((y2/2)-3), 1 - color, 1, 1);
            g15r_pixelBox (rctx, x1+5, y1+((y2/2)+3), x2-5 , y2-6, 1 - color, 1, 1);
            break;
        case 7:
            g15r_pixelBox (rctx, x1, y1, x2, y2 , color, 1, 1);
            g15r_pixelBox (rctx, x1, y1+5, x2 -5, y2, 1 - color, 1, 1);
            break;
        case 8:
            g15r_pixelBox (rctx, x1, y1, x2, y2 , color, 1, 1);
            g15r_pixelBox (rctx, x1+5, y1+5, x2-5 , y1+((y2/2)-3), 1 - color, 1, 1);
            g15r_pixelBox (rctx, x1+5, y1+((y2/2)+3), x2-5 , y2-6, 1 - color, 1, 1);
            break;
        case 9:
            g15r_pixelBox (rctx, x1, y1, x2, y2 , color, 1, 1);
            ...
            break;
        case 10: 
            g15r_pixelBox (rctx, x2-5, y1+5, x2, y1+10 , color, 1, 1);
            g15r_pixelBox (rctx, x2-5, y2-10, x2, y2-5 , color, 1, 1);
            break;
        case 11: 
            g15r_pixelBox (rctx, x1, y1+((y2/2)-2), x2, y1+((y2/2)+2), color, 1, 1);
            break;
        case 12:
            g15r_pixelBox (rctx, x2-5, y2-5, x2, y2 , color, 1, 1);
            break;
    }
}
